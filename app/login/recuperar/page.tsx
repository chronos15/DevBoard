...
export default function RecoverPasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (resetError) setError(toUserFacingError(resetError, "Não foi possível enviar as instruções agora"))
  ...
  return <main className="flex min-h-screen items-center justify-center bg-background px-4 py-10"><div className="w-full max-w-md rounded-2xl bg-card p-6 ring-1 ring-foreground/8 md:p-8">
    ...
    <h1 className="mt-6 text-2xl font-semibold tracking-tight">Recuperar senha</h1><p className="mt-1 text-sm text-muted-foreground">Enviaremos um link seguro para você definir uma nova senha.</p>
