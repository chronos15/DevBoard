...
export async function stageServerShare(formData: FormData, preferredToken?: string): Promise<StagedServerShare> {
  ...
  // V224: o Windows Server é o caminho principal e síncrono. Não esperamos
  ...
  try {
    ...
    if (admin) {
      void writeRemote(admin, manifest, files).catch((error) => {
        console.warn("[TaskBoard/PWA Share V224] Redundância Supabase indisponível; recebimento local preservado", {
          token: manifest.id,
    ...
    if (admin) {
      try {
        ...
        console.error("[TaskBoard/PWA Share V224] Falha no disco local e no fallback Supabase", {
      ...
      console.error("[TaskBoard/PWA Share V224] Falha ao persistir recebimento no disco local", {
        localMessage: localError instanceof Error ? localError.message : String(localError),
      })
    }
