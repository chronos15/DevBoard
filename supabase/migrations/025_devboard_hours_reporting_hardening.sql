-- Devboard · Controle de horas · hardening final
-- Evita dependência de EXECUTE em is_workspace_admin(...) e mantém o relatório
-- seguro para administradores e usuários comuns.
...
begin
  ...
  -- Checagem direta dentro da SECURITY DEFINER: não depende de permissão de
  -- execução em is_workspace_admin(...) e não aceita user_id arbitrário.
  select exists (
    select 1
...
revoke execute on function public.hours_report(timestamptz,timestamptz,uuid,uuid) from public, anon, authenticated;
grant execute on function public.hours_report(timestamptz,timestamptz,uuid,uuid) to authenticated;

-- A policy atual de exclusão de mídia de grupos, criada pelas migrations de chat,
-- chama is_workspace_admin(...) diretamente como authenticated. Como a migration 024
-- fecha esse helper, trocamos por uma variante segura que sempre consulta auth.uid().
create or replace function public.is_current_workspace_admin(p_workspace_id uuid)
returns boolean
language sql
stable
security definer
...
revoke execute on function public.is_current_workspace_admin(uuid) from public, anon, authenticated;
grant execute on function public.is_current_workspace_admin(uuid) to authenticated;
  ...
  and exists (
    ...
    where c.id = public.safe_chat_media_conversation_id(storage.objects.name)
      ...
      and (
        ...
        or public.is_current_workspace_admin(c.workspace_id)
      )
  )
);
